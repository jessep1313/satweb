...
def login_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if empresa:
                # Autenticación contra base de datos de empresa
                try:
                    user = Usuario.objects.using(empresa).get(use_login=username)
                    if user.check_password(password):
                        # Verificamos si es Admin (por ahora solo permitimos admin)
                        if user.use_tipo == 'Admin':
                            # Guardamos en sesión los datos del usuario tenant
                            request.session['user_type'] = 'admin'
                            request.session['empresa_db'] = empresa
                            request.session['user_id'] = user.use_id
                            request.session['user_login'] = user.use_login
                            messages.success(request, f'Bienvenido {user.use_nombre}')
                ...
                # Autenticación como usuario Empresa (superadmin)
