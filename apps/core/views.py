...
def crear_tablas_cliente(empresa_db, rfc_cliente):
    """
    Crea todas las tablas asociadas a un cliente en la base de datos de su empresa.
    Utiliza el RFC del cliente para generar los nombres de tabla.
    Las tablas solo se crean si no existen (IF NOT EXISTS).
    """
    # Sanitizar RFC: solo letras mayúsculas y números
    rfc_clean = re.sub(r'[^a-zA-Z0-9]', '', rfc_cliente)

    # Diccionario con los nombres de tabla y sus definiciones SQL
    tablas = {
        f"cfdiE_{rfc_clean}": """
            CREATE TABLE IF NOT EXISTS `{tabla}` (
                `id` INT AUTO_INCREMENT PRIMARY KEY,
                `rfc_emisor` VARCHAR(255) NULL,
                `rfc_receptor` VARCHAR(255) NULL,
                `folio` VARCHAR(255) NULL,
                `uudi` VARCHAR(255) NULL,
                `fecha_comprobante` DATE NULL,
                `total` VARCHAR(255) NULL,
                `Iva` VARCHAR(255) NULL,
                `Suma` VARCHAR(255) NULL,
                `status_sat` VARCHAR(255) NULL,
        ...
        f"cfdi_{rfc_clean}": """
            CREATE TABLE IF NOT EXISTS `{tabla}` (
                ...
                `estado_factura` VARCHAR(100) NULL,
                `saldo_pendiente` VARCHAR(100) NULL,
                `num_complemento` VARCHAR(100) NULL
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
        """,
        f"efos_historicos_resumen_{rfc_clean}": """
            CREATE TABLE IF NOT EXISTS `{tabla}` (
                `id` INT AUTO_INCREMENT PRIMARY KEY,
        ...
        """,
        f"historicoconstanciaempleado_{rfc_clean}": """
            CREATE TABLE IF NOT EXISTS `{tabla}` (
                `id` INT AUTO_INCREMENT PRIMARY KEY,
                `nombreArchivo` VARCHAR(255) NULL,
                `fechaArchivo` DATE NULL,
                `rfc_proveedor` VARCHAR(255) NULL,
                `rfc_cliente` VARCHAR(255) NULL
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
        """,
        f"historicoconstancia_{rfc_clean}": """
            CREATE TABLE IF NOT EXISTS `{tabla}` (
                `id` INT AUTO_INCREMENT PRIMARY KEY,
                `nombreArchivo` VARCHAR(255) NULL,
                `fechaArchivo` DATE NULL,
        ...
        f"proveedores_{rfc_clean}": """
            CREATE TABLE IF NOT EXISTS `{tabla}` (
                ...
                `fecha_constancia1` DATE NULL,
                `constancia2` INT DEFAULT 0,
                `constancia2_nombre` VARCHAR(255) NULL,
                `fecha_constancia2` DATE NULL,
                `constancia3` INT DEFAULT 0,
                `constancia3_nombre` VARCHAR(255) NULL,
                `fecha_constancia3` DATE NULL,
                `constancia4` VARCHAR(100) NULL,
                `constancia4_nombre` VARCHAR(100) NULL,
                `fecha_constancia4` DATE NULL,
                `rfc_identy` VARCHAR(255) NULL,
                `msjefos` INT DEFAULT 0,
                `url` LONGTEXT NULL,
                `url2` VARCHAR(255) NULL,
                `consultado` INT DEFAULT 0,
                `ano_actual` VARCHAR(255) NULL
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
        """
    }

    with connections[empresa_db].cursor() as cursor:
        for nombre_tabla, sql_template in tablas.items():
            try:
                # Reemplazar placeholder con el nombre real
                sql = sql_template.format(tabla=nombre_tabla)
                cursor.execute(sql)
            except ProgrammingError as e:
                # Si falla la creación, lanzamos excepción
                raise Exception(f"Error creando tabla {nombre_tabla}: {e}")